...
class Agent:
    def __init__(self, seed=42, risk_profile="balanced", observer=None,
                 ...
                 diverse_fixed=False, strategy_version="v2", calibrate_prior=True,
        ...
        self.advanced_search = bool(advanced_search)
    ...
    def _plan(self, env, thorough=False):
        chosen = optimize(self.space, self.posterior, self._pilots, env.remaining_budget,
                          ...
                          local_search=self.advanced_search, resource_prices=self.advanced_search)
    ...
    def _experiments(self, env, initial=False):
        ...
        if self.strategy_version == "v2":
            return self._experiments_v2(env, initial)
    ...
    @staticmethod
    def _positive_normal(mean, sd):
        """E[max(X, 0)] for a normal X; used only as a local VOI approximation."""
        if sd <= 1e-12:
            return max(0.0, mean)
        z = mean / sd
        return sd * math.exp(-0.5 * z * z) / math.sqrt(2 * math.pi) + mean * 0.5 * math.erfc(-z / math.sqrt(2))

    def _experiments_v2(self, env, initial=False):
        normal = [c for c in self._channels if self.posterior.normalized(c)]
        available = normal or self._channels
        reference = max(available, key=lambda c: float(self.space.channels[c]["conversion_multiplier"]))
        reference_scale = float(self.space.channels[reference]["conversion_multiplier"]) if normal else 1.0
        cheapest = min(float(self.space.channels[c]["cost_per_contact"]) for c in self._channels)
        pilot_spent = max(0.0, float(env.total_budget - env.remaining_budget))
        # Pilot information competes with final-channel upgrades. Escalating
        # resource price discourages spending most of the budget on exploration.
        exploration_allowance = max(cheapest, 0.20 * float(env.total_budget))
        budget_price = 1.0 + 8.0 * (pilot_spent / max(1.0, exploration_allowance)) ** 2
        planned = self.report["campaigns"]
        opportunity = max(0.0, sum(v["expected_incremental_net_gain"] or 0.0 for v in planned) /
                          max(1, sum(v["audience_count"] for v in planned)))
        results = []
        for group, audience in self.space.groups.items():
            seen_group = sum(p["group"] == group for p in self._pilots)
            if initial and seen_group:
                continue
            indices = audience.indices[:min(5000, int(env.remaining_contacts) - 1)]
            if not len(indices):
                continue
            baseline = float(self.space.baseline[indices].sum())
            average = float(self.space.baseline[audience.indices].mean())
            targets = [t for t in self.space.tariffs if t != group[0]]
            beliefs = {t: self.posterior.get(group, t, reference) for t in targets}
            for target in targets:
                if (group, target) in self._failed_hypotheses:
                    continue
                mean, variance, prior_pilots = beliefs[target]
                # The competing offer and no additional campaign are both valid
                # alternatives. New information matters when it changes this choice.
                alternative = max([0.0, *[beliefs[t][0] for t in targets if t != target]])
                for channel in available:
                    cost = float(self.space.channels[channel]["cost_per_contact"])
                    multiplier = float(self.space.channels[channel]["conversion_multiplier"]) if normal else 1.0
                    reserve_capacity = min(int(env.remaining_contacts) - 1,
                        int(max(0, env.remaining_budget - cheapest) // cost) if cost > 0 else int(env.remaining_contacts) - 1)
                    if self._pilots and cost > 0:
                        reserve_capacity = min(reserve_capacity, int(max(0.0, exploration_allowance - pilot_spent) // cost))
                    channel_mean = mean * multiplier / reference_scale
                    sizes = sorted({max(10, min(size, reserve_capacity)) for size in (40, 100, 200)})
                    for requested in sizes:
                        actual = min(requested, len(audience.indices))
                        if actual <= 0 or actual > reserve_capacity:
                            continue
                        obs_variance = NOISE_STD ** 2 / actual * (reference_scale / multiplier) ** 2
                        after_variance = variance * obs_variance / (variance + obs_variance)
                        learning_sd = math.sqrt(max(0.0, variance - after_variance))
                        decision_value = self._positive_normal(mean - alternative, learning_sd)
                        current_value = max(0.0, mean - alternative)
                        information_value = baseline * max(0.0, decision_value - current_value)
                        harm = max(0.0, -channel_mean) * average * actual
                        value = information_value - actual * cost * budget_price - harm - opportunity * actual * 0.20
                        # Repeated noisy looks have less chance to add a distinct
                        # useful campaign; do not ban a weak negative pilot.
                        target_repeats = sum(p["target"] == target for p in self._pilots) if initial else 0
                        value /= 1.0 + 0.08 * seen_group + 0.50 * len(prior_pilots) + 0.35 * target_repeats
                        results.append((value, baseline, group, target, channel, requested, actual))
        return sorted(results, key=lambda row: (-row[0], -row[1], row[2], row[3], row[4], row[5]))
    ...
    def act(self, env) -> list[dict]:
        ...
        history = HistoryPrior(Path(__file__).resolve().parents[1] / "data" / "change_tariff.csv", self.use_history,
                               strength=self.history_strength)
        self.posterior = PosteriorTable(history, self.space.channels,
                                        calibrate=self.strategy_version == "v2" and self.calibrate_prior)
        if history.warning:
        ...
        for step in range(max_iterations):
            ...
            reason = (f"{'Diverse initial coverage' if diverse else 'Decision-relevant uncertainty'}; "
                      f"compared sample sizes {'40/100/200' if self.strategy_version == 'v2' else '80/140/200'} and available unsaturated channels; heuristic value {value:.2f} CU.")
            ...
            self.report["prior_calibration"] = self.posterior.calibration_summary()
