...
const topics = [
  { title: 'Как сравнить несколько запусков?', category: 'Результат', text: 'Откройте «Запуски» и отметьте от двух до трёх расчётов, затем нажмите кнопку сравнения. Вы увидите эффект, расходы, охват и различия в предложениях рядом. Для честного сравнения подходов нужны одинаковые данные, номер сценария и лимиты. Интерфейс покажет расхождения; версию алгоритма нужно сверять отдельно, поскольку она не сохранена в старых отчётах.', to: '/runs', action: 'Выбрать запуски' },
  { title: 'Как сохранить сценарий калькулятора и поделиться им?', category: 'Настройки', text: 'В калькуляторе задайте параметры, придумайте название и при желании добавьте заметку. Сохранённый сценарий можно загрузить позже или удалить. Кнопка копирования ссылки передаёт параметры расчёта; получателю нужен тот же набор данных и режим работы. Сценарии хранятся только в этом браузере. Очистка данных сайта удалит их, поэтому нужные варианты лучше дополнительно сохранить ссылкой.', to: '/calculator', action: 'Открыть калькулятор' },
  { title: 'Что показывают осторожный и оптимистичный сценарии?', category: 'Показатели', text: 'Калькулятор меняет предполагаемую вероятность перехода на выбранное число процентных пунктов и пересчитывает эффект. Осторожный вариант — меньший эффект, оптимистичный — больший. Это помогает понять, насколько вывод зависит от предположения. Эти три оценки не являются доверительным интервалом и не показывают вероятность получения прибыли.' },
  { title: 'Где задать название, заметку и избранное?', category: 'Результат', text: 'На странице расчёта откройте «Название и заметки», заполните поля и сохраните. Название появится в истории и отчёте для PDF, заметку можно найти поиском в истории. Избранное помогает быстро вернуться к нужным результатам. Эти пометки хранятся в текущем браузере и не меняют исходные CSV и JSON.', to: '/runs', action: 'Открыть историю' },
  { title: 'Как получить красивый отчёт в PDF?', category: 'Результат', text: 'Откройте завершённый расчёт и нажмите «Отчёт для PDF». Проверьте выводы, показатели и таблицу, затем нажмите «Печать / PDF». В системном окне печати выберите «Сохранить как PDF». Отчёт включает исходный план и вашу сохранённую заметку; эксперименты с исключениями и расчёты калькулятора в него не входят.' },
  ...
  { title: 'Как улучшить план: что добавить или убрать?', category: 'Результат', text: 'Разбор результата помогает найти слабые места: отрицательный эффект, большой разрыв между прогнозом и проверкой, неподтверждённые предложения или нехватку исходных данных. Это рекомендации для проверки, а не изменения плана. В завершённом расчёте на сервере можно отметить кампании для исключения и выполнить отдельную учебную проверку. Она заново учитывает пересечения и расходы, не изменяя исходный план. В демонстрационном режиме этот эксперимент недоступен. Изучите причины выбора и создайте новый расчёт с другим подходом к риску; сравните оба результата в «Запусках». Смена номера сценария меняет условия опыта, поэтому сама по себе не доказывает улучшение.', to: '/runs/new', action: 'Рассчитать другой вариант' },
