...
export function Skeleton({ rows = 3 }: { rows?: number }) { return <div className="skeleton-stack" role="status" aria-label="Загрузка данных"><span className="sr-only">Загружаем данные…</span>{Array.from({ length: rows }, (_, i) => <div className="skeleton" aria-hidden="true" key={i} style={{ width: `${Math.max(35, 100 - i * 12)}%` }} />)}</div> }
