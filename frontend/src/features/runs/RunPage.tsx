...
import { ArrowLeft, ArrowRight, ChevronRight, CircleAlert, Clock3, Download, FileJson2, Info, ListChecks, RefreshCw, X } from 'lucide-react'
import { Link, useParams, useSearchParams } from 'react-router-dom'
import { api } from '../../lib/api/client'
import type { CampaignSpec, CampaignView, RunSnapshot } from '../../lib/api/types'
import { channel, elapsed, money, number, percentage, risk, segment, status, tariff, when } from '../../lib/format'
...
function filterLabel(value: string | null, kind: 'data' | 'calls') {
...
export function RunPage() {
  ...
  return <div className="page-stack run-page">
    ...
    {completed && <>
      <nav className="run-section-nav" aria-label="Разделы результата"><a href="#run-analysis"><Info size={16} aria-hidden="true" /> Разбор результата</a><a href="#campaign-plan"><ListChecks size={16} aria-hidden="true" /> План кампаний <span>{number(run.campaigns.length)}</span></a><a href="#run-research">Что проверили и почему</a><a href="#run-export"><Download size={16} aria-hidden="true" /> Скачать файлы</a></nav>
