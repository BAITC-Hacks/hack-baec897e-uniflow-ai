...
export function RunAnalysis({ run, onOpenCampaign }: { run: RunSnapshot; onOpenCampaign?: (id: string) => void }) {
  ...
  return <section id="run-analysis" className="run-analysis card run-anchor" aria-labelledby="run-analysis-title">
    ...
    <div className="analysis-footnote"><Info size={16} aria-hidden="true" /><div>{complete && <><p>Это сохранённый план для просмотра. В новом подборе можно выбрать уровень осторожности или номер учебного сценария. Кампании и пробные проверки система подбирает сама. Исключение кампаний можно проверить в отдельном эксперименте ниже; исходный план сохраняется.</p><a className="analysis-action" href="/runs/new">Настроить новый подбор<ArrowUpRight size={15} aria-hidden="true" /></a></>}<p>{run.mode === 'demo' ? 'Сейчас показан демонстрационный пример.' : 'Расчёт использует учебные данные.'} Прогноз и локальная проверка не являются фактическим доходом.</p></div></div>
  </section>
}
