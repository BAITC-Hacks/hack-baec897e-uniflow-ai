# Общий контракт двух разработчиков — UniFlow Campaign Studio, v1

Это согласованное ТЗ для будущей реализации, а не описание уже работающего API. Оба разработчика читают этот файл. Backend реализует контракт, frontend использует его в HTTP-клиенте и демонстрационных данных. Контракт конкурсного `Agent.act(env)` остаётся отдельным и имеет приоритет над продуктовой оболочкой.
